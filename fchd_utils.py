...
def data_line_parser(line, dataset):
    xml_path = os.path.join(opt.scutheadb_dataset_root_path,'Annotations/' + line + '.xml')
    src_path = os.path.join(opt.scutheadb_dataset_root_path,'JPEGImages/' + line + '.jpg')
...
def get_phase_data_list(data_list_path, dataset):
    ...
    with open(data_list_path, 'rb') as fp:
        for line in fp.readlines():
            line = str(line, encoding='utf-8')[:11]
