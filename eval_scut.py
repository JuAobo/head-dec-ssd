...
annopath = os.path.join(args.scut_root, 'Annotations', '%s.xml')
imgpath = os.path.join(args.scut_root, 'JPEGImages', '%s.jpg')
imgsetpath = os.path.join(args.scut_root, 'ImageSets',
                          'Main', '{:s}.txt')
devkit_path = args.scut_root
...
def get_voc_results_file_template(image_set, cls):
    # /results/det_test_aeroplane.txt
...
def write_voc_results_file(all_boxes, dataset):
    for cls_ind, cls in enumerate(labelmap):
        print('Writing {:s} SCUT results file'.format(cls))
...
if __name__ == '__main__':
    ...
    # load data
    dataset = SCUTDetection(args.scut_root, set_type,
                           BaseTransform(300, dataset_mean),
                           SCUTAnnotationTransform())
    if args.cuda:
        net = net.cuda()
